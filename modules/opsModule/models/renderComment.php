...
<div  class="total-comment-no">
    <span class='no'><?php echo $this->count_of_comments ? $this->count_of_comments : "No"; ?></span> <label><?php echo \ActiveRecord\Utils::pluralize_if($this->count_of_comments, "comment")?></label>
</div>
<?php endif; ?>
...
<?php else: ?>
        ...
        <div class="col-xs-12 col-sm-10 pull-left" style='margin-left: 20px;'>
                ...
                (function(){
                    ...
                    window.init_comment_txtbox("textarea[name='comment']", {
                        ...
                        success: function(data) { 
                                $(".no-comment").hide();
                                $(data).hide().prependTo(".comments").not('.no-comment').slideDown();
                                var cc = parseInt($(".total-comment-no .no").text());
                                console.log([isNaN(cc), cc]);
                                if(isNaN(cc)) cc = 0;
                                $(".total-comment-no .no").html(cc + 1);
                                if(cc > 2)
...
<div class="comment <?php echo $comment->user_id == $cuid ? "my-comment" : "" ?>" data-commenter="<?php echo sha1($comment->user_id) ?>" data-id="<?php echo $comment->comment_id ?>">
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-10 comment-body pull-left">
            <div class="row">
...
<?php endforeach; ?>
<div class="text-muted text-center no-comment" style="<?php if(count($comments)) echo "display: none;"?>border: 1px solid #DDD;height: 45px;padding-top: 10px;font-weight: bolder" >
    No comment found!
</div>
<?php
    }
...
window.init_comment_found = function() {$(".no-comment:not(:first)").remove();if($(".comment").length) $(".no-comment").hide(); else $(".no-comment").show(); };
window.init_comments = function() {
    window.init_comment_found();
        ...
        $.ajax({
            ...
            success: function(data) {
                if(typeof(data.result) === "undefined")
                    data.result = 0;
                if(data.result) {
                    $(".comment.deleting").slideUp('slow', function(){$(this).remove();window.init_comment_found();});
                    var cc = parseInt($(".total-comment-no .no").text()) - 1;
                    $(".total-comment-no .no").html(cc ? cc : "No");
                    if(cc > 2 || cc === 0)
                        $(".total-comment-no label").text("comments");
                    else
                        $(".total-comment-no label").text("comment");
